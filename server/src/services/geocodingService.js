...
function normalize(result) {
  ...
  const street = [details.road || details.pedestrian || details.footway || details.amenity, details.house_number].filter(Boolean).join(' ');
...
async function nominatim(path, params) {
  ...
  if (!response.ok) throw Object.assign(new Error('Nie udało się ustalić adresu lokalizacji.'), { status: 502 });
