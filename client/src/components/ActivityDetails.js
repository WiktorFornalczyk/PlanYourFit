...
export default function ActivityDetails({ activity, onClose, onEdit, onDelete, onRegenerateRoute }) {
  const [routeBusy, setRouteBusy] = useState(false);
  const [routeCooldown, setRouteCooldown] = useState(false);
  ...
  const changeRoute = async () => {
    if (routeBusy || routeCooldown) return;
    setRouteBusy(true); setRouteCooldown(true);
    window.setTimeout(() => setRouteCooldown(false), 3000);
    await onRegenerateRoute(activity);
    setRouteBusy(false);
  };
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <div className="modal details-modal" role="dialog" aria-modal="true">
      ...
      <div className="details-body">
        ...
        {googleMapsUrl && <div className="route-external-actions"><div className="route-action-buttons"><button className="secondary-button" onClick={changeRoute} disabled={routeBusy||routeCooldown}><Icon name="route"/> {routeBusy?'Generuję…':routeCooldown?'Odczekaj 3 sekundy':'Zmień trasę'}</button><a className="secondary-button" href={googleMapsUrl} target="_blank" rel="noreferrer"><Icon name="route"/> Otwórz trasę w Google Maps</a></div><small>Google Maps może nieznacznie przeliczyć przebieg trasy pieszej.</small></div>}
