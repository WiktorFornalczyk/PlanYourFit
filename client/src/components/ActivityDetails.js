...
function MiniMap({ activity }) {
  ...
  return <div className="mini-map" aria-label="Podgląd mapy">
    <div className="map-road road-a"/><div className="map-road road-b"/><div className="map-road road-c"/>
    {running ? <svg className="route-line" viewBox="0 0 400 180"><path d="M42 138c25-64 76-72 112-41 24 21 21 53 60 50 46-4 27-87 76-92 34-3 41 31 73 21"/><circle cx="42" cy="138" r="7"/><circle cx="363" cy="76" r="7"/></svg> : <span className="map-pin"><Icon name="pin" size={26}/></span>}
    <span className="map-label">{running ? `${activity.details?.actualDistanceKm || activity.details?.targetDistanceKm || 5} km` : activity.locationAddress}</span>
  </div>;
}
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <div className="modal details-modal" role="dialog" aria-modal="true">
      ...
      <div className="details-body">
        ...
        <div className="details-grid"><div className="detail-item"><span className="detail-icon"><Icon name="pin"/></span><div><small>Lokalizacja</small><b>{activity.locationAddress}</b></div></div><div className="detail-item"><span className="detail-icon"><Icon name="clock"/></span><div><small>Czas trwania</small><b>{(() => { const [h,m]=activity.startTime.split(':').map(Number); const [eh,em]=activity.endTime.split(':').map(Number); return `${eh*60+em-h*60-m} min`; })()}</b></div></div>{activity.activityType === 'running' && <><div className="detail-item"><span className="detail-icon"><Icon name="route"/></span><div><small>Dystans</small><b>{activity.details?.actualDistanceKm || activity.details?.targetDistanceKm || 5} km</b></div></div><div className="detail-item"><span className="detail-icon"><Icon name="target"/></span><div><small>Tempo</small><b>{activity.details?.paceMinPerKm || 6}:00 min/km</b></div></div></>}</div>
