...
function createLocalIcs(activities) {
  const esc = (v='') => String(v).replace(/,/g,'\\,').replace(/\n/g,'\\n');
  const dt = (date,time) => `${date.replace(/-/g,'')}T${time.replace(':','')}00`;
  const events = activities.map((a)=>`BEGIN:VEVENT\r\nUID:${a.id}@planyourfit.local\r\nDTSTART:${dt(a.activityDate,a.startTime)}\r\nDTEND:${dt(a.activityDate,a.endTime)}\r\nSUMMARY:${esc(a.title)}\r\nLOCATION:${esc(a.locationAddress)}\r\nDESCRIPTION:${esc(a.note)}\r\nEND:VEVENT`).join('\r\n');
  return `BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//PlanYourFit//PL\r\n${events}\r\nEND:VCALENDAR`;
}

function AppShell({ user, onLogout, children, page, setPage, theme, setTheme, search, setSearch, sportFilter, setSportFilter, onAdd, onExport }) {
...
export default function App() {
  ...
  const exportCalendar = () => { if(!demo){ window.open(api.exportUrl,'_blank'); return; } const blob=new Blob([createLocalIcs(activities)],{type:'text/calendar'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='planyourfit.ics';a.click();URL.revokeObjectURL(url);notify('Kalendarz został wyeksportowany.','success'); };
  ...
  return <><AppShell {...{user,page,setPage,theme,setTheme,search,setSearch,sportFilter,setSportFilter}} onLogout={logout} onAdd={(date)=>setModal({date})} onExport={exportCalendar}>
