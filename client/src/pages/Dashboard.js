...
function Upcoming({ activities, onSelect, currentDate }) {
  const upcoming = activities.filter((a) => a.activityDate >= currentDate).sort((a, b) => `${a.activityDate}${a.startTime}`.localeCompare(`${b.activityDate}${b.startTime}`)).slice(0, 4);
...
function WeeklyChart({ activities, today }) {
  ...
  const values = days.map((d) => activities.filter((a) => a.activityDate === toIsoLocal(d)).reduce((sum, a) => sum + durationMinutes(a), 0));
